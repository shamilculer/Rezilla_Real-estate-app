...
const Inbox = () => {
    ...
    useEffect(() => {
        ...
        socketInstance.on("messagesMarkedAsRead", ({ chatId, userId }) => {
        ...
        return () => {
            socketInstance.off("messagesMarkedAsRead");
                                ...
                                <div className={clsx(`w-full lg:w-1/3 h-[90vh] md:h-[78vh] md:overflow-y-scroll custom-scrollbar`, chatId && "max-sm:hidden")}>
                                    {
                                        chats?.map((chat) => {
                                            return (
                                                ...
                                                className={clsx('flex items-center gap-x-3 p-5 border-b border-gray-300 relative border-l-primary-colour hover:border-l-[5px] cursor-pointer', chatId === chat?._id && "border-l-[5px]")}
                                                >
                                                    ...
                                                    <div className="w-[85%]">
                                                        ...
                                                        {chat.unreadCounts[user._id] > 0 && (
                                                            <div className="absolute bottom-4 right-4 flex items-center justify-center text-xs size-6 bg-red-700 rounded-full text-white  font-medium">{chat.unreadCounts[user._id]}</div>
                                                        )}
                                                    </div>
                                                </Link>
                                            )
